...
if __name__ == "__main__":
	...
	urls = re.findall(r"\^*.*\.memovpn\.vip+",response)
	print(urls)
	# Banner
	print("#")
	print("# Ping VPN servers and choose the best server based on fastest")
	print("# https://github.com/TheEyeOfCyber/memovpn.vip")
	print("#\n")
	# Preparing
	https = True
	...
	print("[+] Preparing ...")
	new_VPN = re.findall(r"\w[a-z]*.*\.memovpn\.vip",str(sorted_VPNs[0]))
	new_VPN = new_VPN[0]
	filename = "memovpn.conf"
	with open(filename, 'r+') as f:
	    text = f.read()
	    text = re.sub('mtl.memovpn.vip', new_VPN, text)
	    f.seek(0)
	    f.write(text)
	    f.truncate()
	print("[+] Done!")
